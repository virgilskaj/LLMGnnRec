...
from Models_Enhanced import MM_Model_Enhanced, Decoder  # Use our enhanced model
...
class EnhancedTrainer(object):
    def __init__(self, data_config):
        # 修改时间格式（替换冒号和空格）
        self.task_name = "%s_%s_%s_enhanced" % (datetime.now().strftime('%Y-%m-%d_%H-%M-%S'), args.dataset, args.cf_model)
        self.logger = Logger(filename=self.task_name, is_debug=args.debug)
        self.logger.logging("PID: %d" % os.getpid())
        self.logger.logging("Enhanced Model with EmerG GNN Integration")
        ...
        # Use enhanced model instead of original MM_Model
        self.model_mm = MM_Model_Enhanced(
        ...
        # Enhanced optimizer with additional parameters
        ...
        self.logger.logging("Enhanced model initialized successfully")
    ...
    def train(self):
        ...
        self.logger.logging("Starting enhanced training with EmerG GNN integration...")
        ...
        for epoch in range(args.epoch):
            ...
            for idx in tqdm(range(n_batch)):
                ...
                # Enhanced loss with item-specific graph regularization
                graph_reg_loss = 0.0
                if hasattr(self.model_mm, 'graph_generator') and self.model_mm.graph_generator is not None:
                    # Add regularization for graph generator parameters
                    for param in self.model_mm.graph_generator.parameters():
                        graph_reg_loss += torch.norm(param, 2)
                    graph_reg_loss *= getattr(args, 'graph_reg_weight', 0.01)
            ...
            if ret['recall'][1] > best_recall:
                ...
                self.logger.logging("Enhanced Model - Test_Recall@%d: %.5f,  precision=[%.5f], ndcg=[%.5f]" % (eval(args.Ks)[1], test_ret['recall'][1], test_ret['precision'][1], test_ret['ndcg'][1]))
                stopping_step = 0
        ...
        self.logger.logging("Enhanced model training completed")
...
if __name__ == '__main__':
    ...
    print("Starting Enhanced LLMRec with EmerG GNN Integration")
    trainer = EnhancedTrainer(data_config=config)
