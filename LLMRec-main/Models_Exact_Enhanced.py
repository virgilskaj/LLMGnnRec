...
class MM_Model_Exact_Enhanced(nn.Module):
    """
    EXACT copy of original MM_Model with OPTIONAL EmerG-inspired enhancements
    When enhancement is disabled, this should produce IDENTICAL results to original
    """
    def __init__(self, n_users, n_items, embedding_dim, weight_size, dropout_list, image_feats, text_feats, user_init_embedding, item_attribute_dict):
        ...
        # OPTIONAL EmerG-inspired enhancements (only added if explicitly enabled)
        self.enable_enhancement = getattr(args, 'use_enhanced_gnn', False)
        if self.enable_enhancement:
            # Very lightweight enhancements that won't break existing logic
            self.item_feature_gate = nn.Parameter(torch.ones(2))  # For image and text
            self.cross_modal_weight = nn.Parameter(torch.tensor(0.01))  # Very small initial weight
            
            # Simple feature refinement layers
            self.image_enhance = nn.Linear(args.embed_size, args.embed_size, bias=False)
            self.text_enhance = nn.Linear(args.embed_size, args.embed_size, bias=False)
            
            # Initialize enhancement layers to identity
            nn.init.eye_(self.image_enhance.weight)
            nn.init.eye_(self.text_enhance.weight)
    ...
    def forward(self, ui_graph, iu_graph, image_ui_graph, image_iu_graph, text_ui_graph, text_iu_graph):
        ...
        # OPTIONAL EmerG-inspired enhancement (ONLY if enabled and VERY conservative)
        if self.enable_enhancement:
            # Apply minimal enhancement to item features only
            enhanced_image_item_feats = self.image_enhance(image_item_feats)
            enhanced_text_item_feats = self.text_enhance(text_item_feats)
            
            # Cross-modal interaction (very light)
            cross_modal_signal = torch.sigmoid(self.cross_modal_weight) * (
                F.normalize(enhanced_image_item_feats, p=2, dim=1) * 
                F.normalize(enhanced_text_item_feats, p=2, dim=1)
            ).sum(dim=1, keepdim=True)
            
            # Apply gated enhancement (learnable gates)
            image_gate = torch.sigmoid(self.item_feature_gate[0])
            text_gate = torch.sigmoid(self.item_feature_gate[1])
            
            # Very conservative enhancement (max 5% change)
            image_item_feats = image_item_feats + image_gate * 0.05 * enhanced_image_item_feats
            text_item_feats = text_item_feats + text_gate * 0.05 * enhanced_text_item_feats
            
            # Add cross-modal signal (very small)
            image_item_feats = image_item_feats + 0.01 * cross_modal_signal * enhanced_image_item_feats
            text_item_feats = text_item_feats + 0.01 * cross_modal_signal * enhanced_text_item_feats
