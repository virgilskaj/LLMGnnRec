...
class EmerGLiteEnhancer(nn.Module):
    """
    Lightweight implementation of EmerG's core idea:
    Learn item-specific feature interaction patterns
    """
    def __init__(self, embed_size, n_items):
        super(EmerGLiteEnhancer, self).__init__()
        self.embed_size = embed_size
        self.n_items = n_items
        
        # Item-specific feature interaction weights (core EmerG idea)
        # Each item learns its own way to combine image and text features
        self.item_interaction_weights = nn.Embedding(n_items, 4)  # 4 interaction patterns
        
        # Initialize to neutral values
        nn.init.constant_(self.item_interaction_weights.weight, 0.25)  # Equal weights initially
        
        # Feature refinement networks (very simple)
        self.image_refine = nn.Linear(embed_size, embed_size, bias=False)
        self.text_refine = nn.Linear(embed_size, embed_size, bias=False)
        
        # Initialize to identity (no change initially)
        nn.init.eye_(self.image_refine.weight)
        nn.init.eye_(self.text_refine.weight)

    def forward(self, image_feats, text_feats, item_indices=None):
        """
        Apply item-specific enhancement
        Args:
            image_feats: [n_items, embed_size]
            text_feats: [n_items, embed_size]
            item_indices: specific items to enhance (optional)
        """
        if item_indices is None:
            item_indices = torch.arange(self.n_items).cuda()
        
        # Get item-specific interaction weights
        interaction_weights = self.item_interaction_weights(item_indices)  # [n_items, 4]
        interaction_weights = F.softmax(interaction_weights, dim=-1)
        
        # Apply item-specific feature refinement
        refined_image = self.image_refine(image_feats)
        refined_text = self.text_refine(text_feats)
        
        # Item-specific feature combination (EmerG's core idea)
        # Each item learns its own optimal way to combine features
        w1, w2, w3, w4 = interaction_weights[:, 0:1], interaction_weights[:, 1:2], interaction_weights[:, 2:3], interaction_weights[:, 3:4]
        
        enhanced_image = (w1 * image_feats + w2 * refined_image + 
                         w3 * text_feats * 0.1 +  # Cross-modal signal
                         w4 * refined_text * 0.1)
        
        enhanced_text = (w1 * text_feats + w2 * refined_text + 
                        w3 * image_feats * 0.1 +  # Cross-modal signal
                        w4 * refined_image * 0.1)
        
        return enhanced_image, enhanced_text


class MM_Model_EmerG_Lite(nn.Module):
    """
    MM_Model with EmerG Lite enhancement
    Preserves 100% of original logic, adds minimal enhancements
    """
    def __init__(self, n_users, n_items, embedding_dim, weight_size, dropout_list, image_feats, text_feats, user_init_embedding, item_attribute_dict):
        ...
        # EmerG Lite enhancer (only if enabled)
        self.use_enhancement = getattr(args, 'use_enhanced_gnn', False)
        if self.use_enhancement:
            self.emerg_enhancer = EmerGLiteEnhancer(args.embed_size, n_items)
    ...
    def forward(self, ui_graph, iu_graph, image_ui_graph, image_iu_graph, text_ui_graph, text_iu_graph):
        ...
        # EmerG Lite Enhancement: Apply BEFORE graph propagation (key insight!)
        if self.use_enhancement:
            try:
                enhanced_image_feats, enhanced_text_feats = self.emerg_enhancer(image_feats, text_feats)
                # Use enhanced features for propagation
                image_feats = enhanced_image_feats
                text_feats = enhanced_text_feats
            except Exception as e:
                print(f"EmerG Lite enhancement failed: {e}")
        ...
        # EXACTLY as original - ID embeddings and propagation
